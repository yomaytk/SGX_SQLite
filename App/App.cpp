...
// store binary file data in a buffer
static std::pair<size_t, uint8_t*> binary_file_to_buf(const char *sealed_db_file, sgx_enclave_id_t *eid) {
    ...
    if(buf == NULL)
    {
        ...
        sgx_destroy_enclave(*eid);
        std::cout << "Error Code: 2-2" << endl;
        exit(-1);
    }
    if (read_file_to_buf(sealed_db_file, buf, fsize) == false)
    {
        std::cout << "fsize: " << fsize << "file: " << sealed_db_file << endl;
        std::cout << "Failed to read the sealed data blob from \"" << sealed_db_file << "\"" << std::endl;
        free(buf);
        sgx_destroy_enclave(*eid);
        std::cout << "Error Code: 2-3" << endl;
        exit(-1);
    }
    return std::make_pair(fsize, buf);
}
...
int main(int argc, char *argv[]){
    ...
    snprintf(sealed_db_file, 100, "%s.signed", dbname);
    std::cout << "sealed database file: " << sealed_db_file << endl;

    char *find_file_cmd = (char *)malloc(CMD_DEFSIZE);
    snprintf(find_file_cmd, 100, "if test -e %s; then\n\techo 1\nelse\n\techo 2\nfi\0", sealed_db_file);
    std::cout << find_file_cmd << endl;

    FILE *findfile_fp = NULL;
    char buf[10];
    bool dfae = false;

    if ((findfile_fp = popen(find_file_cmd, "r")) != NULL) {
        if (fgets(buf, sizeof(buf), findfile_fp) != NULL) {
            // get 1 or 2
            dfae = buf[0] == '1';
            std::cout << "buf: " << buf[0] << endl;
            assert(fgets(buf, sizeof(buf), findfile_fp) == NULL);
        } else {
            std::cout << "find file command fail." << endl;
            return -1;
        }
        pclose(findfile_fp);
    } else {
        std::cout << "find_file_cmd failed." << endl;
        return -1; 
    }
    ...
    if (dfae) {
        // Unsealing 
        cout << dbname << " already exist." << endl;
        // Read the sealed blob from the file
        size_t fsize = get_file_size(sealed_db_file);
        if (fsize == (size_t)-1) {
            std::cout << "Failed to get the file size of \"" << sealed_db_file << "\"" << std::endl;
        ...
        uint8_t *tmp_buf = (uint8_t *)malloc(fsize); 
        if(tmp_buf == NULL) {
            std::cout << "Out of memory" << std::endl;
            sgx_destroy_enclave(eid);
            return -1;
        ...
        // store enctypted data to buffer
        if (read_file_to_buf(sealed_db_file, tmp_buf, fsize) == false) {
            std::cout << "Failed to read the sealed data blob from \"" << sealed_db_file << "\"" << std::endl;
            free(tmp_buf);
            sgx_destroy_enclave(eid);
            return -1;
        ...
        sgx_status_t retval;
        ret = unseal_data(eid, &retval, tmp_buf, fsize);
        if (ret != SGX_SUCCESS) {
            free(tmp_buf);
            sgx_destroy_enclave(eid);
            return -1;
    ...
    // Sealing dbname file to dbname.signed
    auto dbdata = binary_file_to_buf(dbname, &eid);
    size_t fsize = dbdata.first;
    uint8_t *db_buf = dbdata.second;
    
    uint32_t sealed_data_size = 0;
    ret = get_sealed_data_size(eid, &sealed_data_size, fsize);
    if (ret != SGX_SUCCESS) {
        sgx_destroy_enclave(eid);
        cout << "Error Code: 3-1" << endl;
        return -1;
    }

    uint8_t *sealed_data_buf = (uint8_t *)malloc(fsize);
    if (sealed_data_buf == NULL) {
        sgx_destroy_enclave(eid);
        cout << "Error Code: 3-2" << endl;
        return -1;
    }
    
    uint8_t *tmp_buf = (uint8_t *)malloc(sealed_data_size);

    ret = seal_data(eid, &retval, tmp_buf, sealed_data_size, db_buf, fsize);
    if (ret != SGX_SUCCESS) {
        free(sealed_data_buf);
        free(tmp_buf);
        sgx_destroy_enclave(eid);
        std::cout << "Error Code: 3-3" << endl;
        return -1;
    } else if(retval != SGX_SUCCESS) {
        free(sealed_data_buf);
        free(tmp_buf);
        sgx_destroy_enclave(eid);
        std::cout << "Error Code: 3-4" << endl;
        return -1;
    }

    // Save the sealed blob
    if (write_buf_to_file(sealed_db_file, tmp_buf, sealed_data_size, 0) == false)
    {
    ...
    std::cout << "Sealing database data succeeded." << std::endl;
